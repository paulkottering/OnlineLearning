...
def plot_lineplot_with_errorbars(varying_values_choice,varying_parameters, varying_values, data_ne, data_p, data_ni, fixed_parameters,std_ne,std_p,std_ni):
    sns.set()
    ...
    for i in range(3):
        ...
        axes[i].errorbar(varying_values[0], data_ne[:, i], yerr=std_ne[:, i], fmt=fmts[i], capsize=5, label=varying_values[1][i],color=colors[0])
        axes[i].errorbar(varying_values[0], data_ni[:, i], yerr=std_ni[:, i], fmt=fmts[i], capsize=5, label=varying_values[1][i],color=colors[1])
        axes[i].set_xticks(varying_values[0])
        axes[i].set_xticklabels(varying_values[0])
    ...
    labels = ["Nash Regret","Nikaido-Isoda Regret"]

    legend_elements = [
        plt.Line2D([0], [0],lw=4,color=colors[i], label=labels[i])
        for i in range(2)
    ]
    ...
    if not os.path.exists("Figures"):
        os.makedirs("Figures")
    title = "Figures/6_3.png".format(varying_parameters[0], varying_parameters[1])
...
if __name__ == "__main__":
    varying_parameters = ["constant", "noise"]
    ...
    plot_lineplot_with_errorbars(varying_values_choice,varying_parameters, varying_values, data_ne, data_p, data_ni, fixed_parameters,std_ne,std_p,std_ni )