...
def plot_lineplot_with_errorbars(varying_values_choice, varying_parameters, varying_values, data_ne, data_p, data_ni,
    ...
    for n in range(3):
        for p in range(3):
            ...
            # Add column titles
            if n == 0:
                axes[n, p].set_title(f'Players = {p + 2}')
    ...
    if not os.path.exists("lineplots"):
        os.makedirs("lineplots")
    title = "lineplots/cooperative_{}_{}_lineplot.png".format(varying_parameters[0], varying_parameters[1])
...
def load_data(varying_parameters, fixed_parameters, varying_values_choice):
    ...
    data_ne = np.zeros(tuple(len(values) for values in varying_values)+(50,5000))
    data_p = np.zeros(tuple(len(values) for values in varying_values)+(50,5000))
    data_ni = np.zeros(tuple(len(values) for values in varying_values)+(50,5000))
