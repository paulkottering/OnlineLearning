...
def plot_lineplot_with_errorbars(varying_values_choice,varying_parameters, varying_values, data_ne, data_p, data_ni, fixed_parameters,std_ne,std_p,std_ni):
    ...
    for i in range(3):
        ...
        for s in range(2):
            ...
            axes[1,i].errorbar(varying_values[0], data_ni[:, i, s], yerr=std_ni[:, i, s], fmt=fmts[s], capsize=5, label=varying_values[2][s],color = colors[s])
    ...
    axes[1,0].set_ylabel('Cumulative Nikaido-Isoda Regret')
    ...
    if not os.path.exists("Figures"):
        os.makedirs("Figures")
    title = "Figures/6_6.png".format(varying_parameters[0], varying_parameters[1])
